...
// ── 프롬프트 헬퍼 ────────────────────────────────────────
const TONE_MAP = {
  '정보성': '유용한 정보를 명확하고 쉽게 전달하는 정보성 문체로 작성합니다. 수치·사실·팁을 적극 활용합니다. 모든 문장은 ~합니다/~입니다/~됩니다/~습니다 형태의 존댓말 종결 어미로 끝맺습니다.',
  '친근한': '독자와 대화하듯 편안하고 친근한 존댓말 문체로 작성합니다. 이모지는 사용하지 않습니다. 모든 문장은 ~합니다/~입니다/~됩니다/~습니다 형태의 존댓말 종결 어미로 끝맺습니다.',
  '전문적': '전문성과 신뢰를 드러내는 격식체 존댓말로 작성합니다. 전문 용어를 적절히 사용하되 독자가 이해할 수 있게 설명합니다. 모든 문장은 ~합니다/~입니다/~됩니다/~습니다 형태의 존댓말 종결 어미로 끝맺습니다.',
  '감성적': '독자의 공감을 이끌어내는 따뜻하고 감성적인 존댓말 문체로 작성합니다. 경험과 감정을 담아 서술합니다. 모든 문장은 ~합니다/~입니다/~됩니다/~습니다 형태의 존댓말 종결 어미로 끝맺습니다.',
  '후기성': '실제 수강생·합격자의 생생한 후기 형식으로. "처음엔 정말 막막했어요", "코칭 받고 나서 달라지더라고요" 같은 전후 변화를 중심으로, 구체적 에피소드와 감정의 흐름을 담아 신뢰감 있게 서술합니다. 모든 문장은 ~했어요, ~이었어요, ~달라졌어요, ~느꼈어요, ~하더라고요, ~그렇죠, ~맞죠 같은 ~요/~죠 형태의 구어체 종결 어미로 끝맺습니다. ~합니다, ~입니다, ~했다, ~이다 같은 격식체·평서체 어미는 사용하지 않습니다.'
};
...
8. 글 맨 끝에 빈 줄 후 태그를 추가합니다: \`태그: #태그1 #태그2 #태그3 #태그4 #태그5\`
9. **종결 어미 다양화(필수):** "입니다", "합니다"를 연속으로 2회 이상 사용하지 않습니다. ~다/~했다/~이다/~된다/~겠다/~싶다/~보인다/~있다 등 다양한 종결 어미를 혼용합니다.
10. **접속사 남발 금지:** "그리고", "또한", "그래서"를 단락 내에서 연속으로 사용하지 않습니다. 접속사 없이 문장 구조로 자연스럽게 이어지도록 합니다.
11. **근거 없는 단정 표현 금지:** "100% 효과", "반드시 ~됩니다", "무조건 ~합니다", "확실히 ~보장" 등 근거가 없는 절대적 단정 표현은 사용하지 않습니다. 대신 "실제로 ~하는 경우가 많다", "대부분 ~로 이어진다", "경험상 ~달라진다" 등 경험·경향 기반 표현을 사용합니다.${seoBlock}
...
"입니다", "합니다"를 연속 2회 이상 사용하지 않고 종결 어미를 다양하게 씁니다.
"그리고", "또한", "그래서"를 단락 내에서 연속 사용하지 않습니다.
"100% 효과", "반드시 ~됩니다", "무조건", "확실히 보장" 등 근거 없는 단정 표현은 사용하지 않습니다.
모든 글은 한국어로만 작성합니다.`,
